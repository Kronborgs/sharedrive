  ...
  return (
    <section className="rounded-xl border border-zinc-200 dark:border-[#2d3148] bg-white dark:bg-[#1a1d27] p-5 space-y-5">
      ...
      <div className="space-y-3 border-t border-zinc-100 dark:border-[#2d3148] pt-4">
        ...
        <div className="flex items-center gap-2"><span className="text-xs text-zinc-500 dark:text-slate-400 w-20 shrink-0">{model.t('backup.serverUrl')}</span><code className="flex-1 text-xs font-mono bg-zinc-50 dark:bg-[#0f1117] border border-zinc-200 dark:border-[#2d3148] rounded px-2 py-1 truncate">{window.location.origin}</code><button type="button" onClick={() => { ignorePromise(navigator.clipboard.writeText(window.location.origin)); toast.success(model.t('backup.urlCopied')) }} className="shrink-0 p-1.5 rounded border border-zinc-200 dark:border-[#2d3148] hover:bg-zinc-50 dark:hover:bg-[#2d3148] transition-colors" title={model.t('backup.copyUrl')}><Copy size={12} /></button></div>
        <div className="flex items-center gap-2"><span className="text-xs text-zinc-500 dark:text-slate-400 w-20 shrink-0">{model.t('backup.yourUserId')}</span><code className="flex-1 text-xs font-mono bg-zinc-50 dark:bg-[#0f1117] border border-zinc-200 dark:border-[#2d3148] rounded px-2 py-1 truncate">{model.buddyConfig?.user_id ?? '…'}</code><button type="button" onClick={() => { if (model.buddyConfig?.user_id) { ignorePromise(navigator.clipboard.writeText(model.buddyConfig.user_id)); toast.success(model.t('backup.userIdCopied')) } }} className="shrink-0 p-1.5 rounded border border-zinc-200 dark:border-[#2d3148] hover:bg-zinc-50 dark:hover:bg-[#2d3148] transition-colors" title={model.t('backup.copyUserId')}><Copy size={12} /></button></div>
        <div className="flex items-center gap-2"><span className="text-xs text-zinc-500 dark:text-slate-400 w-20 shrink-0">{model.t('backup.receiveTokenLabel')}</span>{model.buddyConfig?.has_receive_token ? <span className="flex-1 text-xs font-mono text-zinc-500 dark:text-slate-400">{model.buddyConfig.receive_token_prefix}••••••••••••••••••••••••••••••••••••</span> : <span className="flex-1 text-xs text-zinc-400">{model.t('backup.notGeneratedYet')}</span>}<button type="button" onClick={() => model.generateReceiveTokenMutation.mutate()} disabled={model.generateReceiveTokenMutation.isPending} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border border-zinc-200 dark:border-[#2d3148] hover:bg-zinc-50 dark:hover:bg-[#2d3148] transition-colors disabled:opacity-50"><RefreshCw size={10} /> {model.buddyConfig?.has_receive_token ? model.t('backup.rotate') : model.t('backup.generate')}</button>{model.buddyConfig?.has_receive_token && <button type="button" onClick={() => { if (confirm(model.t('backup.revokeReceiveToken'))) model.revokeReceiveTokenMutation.mutate() }} disabled={model.revokeReceiveTokenMutation.isPending} className="shrink-0 flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors disabled:opacity-50"><Trash2 size={10} /> {model.t('backup.revoke')}</button>}</div>
        {model.newReceiveToken && <div className="rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 p-4 space-y-2"><div className="flex items-start gap-2 text-amber-700 dark:text-amber-400"><AlertTriangle size={14} className="mt-0.5 shrink-0" /><p className="text-xs font-medium">{model.t('backup.receiveTokenSaveNow')}</p></div><div className="flex items-center gap-2"><code className="flex-1 text-xs font-mono bg-white dark:bg-[#0f1117] border border-amber-200 dark:border-amber-800 rounded px-3 py-2 break-all text-zinc-800 dark:text-slate-200 select-all">{model.newReceiveToken}</code><button type="button" onClick={model.handleCopyReceiveToken} className="shrink-0 p-2 rounded-lg border border-zinc-200 dark:border-[#2d3148] hover:bg-zinc-50 dark:hover:bg-[#2d3148] transition-colors" title={model.t('backup.copyReceiveToken')}>{model.receiveTokenCopied ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}</button></div></div>}
      </div>
    </section>
  )
}
      ...
      {model.buddyConfig?.peer_configured && (
        <section className="rounded-xl border border-zinc-200 dark:border-[#2d3148] bg-white dark:bg-[#1a1d27] p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => ignorePromise(model.refetchPushedArchives())} className="p-1 rounded hover:bg-zinc-100 dark:hover:bg-[#2d3148] transition-colors text-zinc-400" title="Opdater liste">
              <RefreshCw size={13} />
            </button>
          </div>
