...
function FolderPicker({
  ...
  const { data: rootFiles } = useQuery({
    ...
    queryFn: ({ signal }) =>
      api.get<{ items: FileItem[] }>('/api/v1/files', signal).then(r => r.items ?? []),
    select: (items) => (items as unknown as FileItem[]).filter((f) => f.is_folder),
    enabled: open,
  })
  ...
  return (
    <div className="space-y-1">
      ...
      >
        ...
        {allChecked ? 'All files' : `${selectedIDs.length} folder(s) selected`}
      </button>
      ...
      {open && (
        <div className="rounded-lg border border-zinc-200 dark:border-[#2d3148] bg-zinc-50 dark:bg-[#0f1117] p-3 space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-xs text-zinc-700 dark:text-slate-300">All files</span>
          </label>
          {folders.length > 0 && (
            <div className="border-t border-zinc-200 dark:border-[#2d3148] pt-2 space-y-1">
              {folders.map(f => (
                <label key={f.id} className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-zinc-700 dark:text-slate-300 truncate">{f.name}</span>
                </label>
              ))}
            </div>
          )}
          {folders.length === 0 && (
            <p className="text-xs text-zinc-400">No folders at root level</p>
          )}
        </div>
      )}
    </div>
