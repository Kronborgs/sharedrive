...
import { LayoutList, LayoutGrid, Upload, FolderPlus, ChevronRight, Home } from 'lucide-react'
...
function FilesPage() {
  ...
  const createFolder = useMutation({
    mutationFn: (name: string) => api.post('/api/v1/files/folders', { name, parent_id: folderId }),
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['files', folderId] }),
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto px-1" onClick={() => { setSelected(new Set()); setContextMenu(null) }}>
          {isLoading ? (
            ...
            <FileList items={sorted} selectedIds={selected} onSelect={handleSelect} onOpen={handleOpen} onContextMenu={(item, x, y) => setContextMenu({ item, x, y })} />
          ) : (
